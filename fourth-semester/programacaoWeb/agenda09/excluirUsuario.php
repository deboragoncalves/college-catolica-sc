<?php
    ...
    if(isset($_GET['id'])) {
        ...
        $resultUsuarioInteresse = $connection->query($consultaUsuarioInteresse);
        
        if (!$resultUsuarioInteresse) {
        ...
        if ($resultUsuarioInteresse->num_rows > 0) {
            while($row = $resultUsuarioInteresse->fetch_assoc()) {
                ...
                if (!is_null($usuarioId) && !empty($usuarioId)) {
                    $deletarUsuarioInteresse = "DELETE FROM usuario_interesse WHERE fk_usuario_id = $usuarioId";
                    $resultadoInteresse = mysqli_query($connection, $deletarUsuarioInteresse);
